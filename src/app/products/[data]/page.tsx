...
export async function getProductData(data:string) {
  const productsData = await client.fetch(`*[ _type == 'product && _id == ${data} ']{_id,name,type,price,description,care[],image[]}`);
  return productsData;
}
...
export default async function page({ params }: { params: { data: string } }) {
  const data :Product[]  = await getProductData(params.data);
  return (
    <div>My Post: {params.data}</div>
